...
class Solution {
    public int[] topKFrequent(int[] nums, int k) {
        ...
        PriorityQueue<Map.Entry<Integer, Integer>> kTopFrequentElements = new PriorityQueue<>((a, b) -> a.getValue() - b.getValue());

        for (Map.Entry<Integer, Integer> map : frequency.entrySet()) {
            kTopFrequentElements.add(map);
            if (kTopFrequentElements.size() > k) {
                kTopFrequentElements.poll();
            }
        }
        
        int[] result = new int[k];
        for (int i = 0; i < k; i++) {
            result[i] = kTopFrequentElements.poll().getKey();
        }
